    ...
    while connect:
        ...
        if mes == 'Order':
            ...
            if command_order == "update_food":
                ...
                file = 'order.json'
                order = json.loads(InFile_Menu(file))
                if checkrep2 == "Not_order":
                    continue
                if checkrep2 == "Check_time":
                    for i in range(0, len(order)):
                        if (order[i]["NumberDesk"] == t) and order[i]["Static"] == "No":
                            #gui thoi gian de check xem co hop le hay khong
                            now = datetime.now()
                            current_time = now.time()
                            order_time = order[i]["Time"]
                            ...
                            duration = t2 - t1 
                            if  duration >= limit:
                                conn.send('Over_time'.encode(FORMAT))
                                continue
                            else:
                                conn.send('Suitable'.encode(FORMAT))
                                for i in range(0, len(order)):
                                    if (order[i]["NumberDesk"] == t) and order[i]["Static"] == "No":
                                         #gui thoi gian de check xem co hop le hay khong
                                        ...
                                        break
                                start_object =  conn.recv(SIZE).decode(FORMAT)   
                                if start_object == "Available":
