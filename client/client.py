...
from tkinter import VERTICAL, Canvas, Frame, Label, Scrollbar, Spinbox, Widget, ttk, Spinbox 
...
def Update_List_Food(client, order, data, ms):
    ...
    if (ms.counter <= 0):
        ...
        client.send('Check_time'.encode(FORMAT))
    rep_check_time = client.recv(SIZE).decode(FORMAT)
    if rep_check_time == 'Over_time':
        order.destroy()
    ...
    for i in range(0,len(data)):
        valuecheck.append(tk.IntVar())
        ttk.Checkbutton(frame, text = data[i]['NameFood'], onvalue=1, offvalue=0, variable=valuecheck[i]).grid(column = 0 , row = i + 2, sticky='w' )
    tk.Button(dish_order, text = "Input amount of dishes", font = 'TimeNewRoman',command = partial(Update_Next_amount, client, data, dish_order, list_food, valuecheck, list_order, list_amount, ms)).grid(row = 3, column = 2)
    dish_order.protocol('WM_DELETE_WINDOW', partial(Do_when_press_x_in_order_update, dish_order, client, ms))
    dish_order.mainloop()


def Update_Next_amount(client, data, dish_order, list_food, valuecheck, list_order, list_amount, ms):
    dish_order.destroy()
...
def Pay_food(client, ms):
    ...
    #
    tk.Label(pay,text='PAY', font = ('TimeNewRoman', 20)).grid(column=0, row = 0, sticky='n', padx= 125)
    #in ra man hinh tin va tien them
    tk.Label(pay,text="Your bill is " + str(money[0]) + "+ " + str(money[1] - money[0]) +" money you adding", font = ('TimeNewRoman', 25)).grid(column=0, row = 1, sticky='n', padx=125)
    #
